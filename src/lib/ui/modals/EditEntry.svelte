<script lang="ts">
	...
	let showing_initial_image = true;
						...
						{#if data_type === 'Image'}
							<div class="flex flex-col gap-2 w-full h-full relative">
								...
								<div class="flex">
									...
									{:else if showing_initial_image}
										<div>
											<img class="bg-cover max-h-[200px]" src={value[field_name]} alt="" />
											<button
												on:click|preventDefault={() => {
													loaded_preview = false;
													showing_initial_image = false;
									...
									{/if}
									<div class="relative w-full" hidden={showing_initial_image || loaded_preview}>
										<label
											for={field_name}
