...
<div on:click={()=>{ window.location.href=`/dashboard/container/${name}` }} class="rounded-xl  w-60 h-60 border border-slate-600 cursor-pointer text-center items-center justify-center flex hover:scale-100 hover:-translate-y-1 transition-all" >
    <div class=" space-y-6 w-full h-full flex flex-col items-center rounded-xl p-3" >
        ...
        <button class="flex hover:text-red-500 justify-center hover:scale-105 hover:-translate-y-1 transition-all" on:click={deleteContainer} >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class={`w-6 h-6 ${deleting?" animate-pulse":""} `}>
